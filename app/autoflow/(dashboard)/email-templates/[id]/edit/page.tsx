...
import { getEmailTemplate, updateEmailTemplate } from "@/app/autoflow/actions/email-templates"
  ...
  useEffect(() => {
    async function fetchTemplate() {
      try {
        ...
        if (isNaN(id)) {
          return router.push('/autoflow/email-templates')
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/autoflow/email-templates/${templateId}`)
      } else {
    ...
    <div className="flex flex-col gap-4 p-4 md:gap-8 md:p-8">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" asChild>
          <Link href={templateId ? `/autoflow/email-templates/${templateId}` : '/autoflow/email-templates'}>Cancel</Link>
        </Button>
      </div>
