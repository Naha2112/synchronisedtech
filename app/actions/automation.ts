...
export async function getAutomationWorkflows() {
  ...
  try {
    const workflows = (await query("SELECT * FROM automation_workflows WHERE created_by = ? ORDER BY updated_at DESC", [
      user.id,
...
export async function getAutomationWorkflow(id: number) {
  ...
  try {
    const workflows = (await query("SELECT * FROM automation_workflows WHERE id = ? AND created_by = ?", [
      id,
      user.id,
    ])) as AutomationWorkflow[]
  ...
  try {
    ...
    try {
      ...
      const [workflowResult] = (await connection.execute(
        ...
        VALUES (?, ?, ?, ?, ?)`,
        [data.name, data.description, data.trigger_type, data.is_active, user.id],
      )) as any
  ...
  try {
    ...
    try {
      ...
      await connection.execute(
        ...
        [data.name, data.description, data.trigger_type, data.is_active, id, user.id],
      )
...
export async function toggleWorkflowStatus(id: number, isActive: boolean) {
  ...
  try {
    await query("UPDATE automation_workflows SET is_active = ? WHERE id = ? AND created_by = ?", [
      isActive,
      id,
      user.id,
    ])
...
export async function deleteAutomationWorkflow(id: number) {
  ...
  try {
    await query("DELETE FROM automation_workflows WHERE id = ? AND created_by = ?", [id, user.id])
